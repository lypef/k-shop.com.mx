<?php
    ...
    $con = db_conectar();  
    $sales = mysqli_query($con,"SELECT u.nombre, c.nombre, v.descuento, v.fecha, v.cobrado, v.fecha_venta, v.folio, v.t_pago FROM folio_venta v, users u, clients c WHERE v.vendedor = u.id and v.client = c.id and v.open = 0 and v.cut_global = 0 ");
    ...
    while($row = mysqli_fetch_array($sales))
    {
        ...
        <td class="item-des uppercase"><center><p>'.strtoupper($row[7]).'</p></center></td>
        </tr>
        ';
    }
    ...
    <hr>
    <h4><center>CORTE X GLOBAL</center></h4>
    ...
    mysqli_query($con,"UPDATE `folio_venta` SET `cut_global` = '1' WHERE open = 0;");
?>